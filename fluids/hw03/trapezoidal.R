...
for(i in 2:length(d)){
    ...
    #Sf[i] <- Sff(d[i])
    #E[i] <- Ef(d[i])
    x[i] <- x[i-1]-(Ef(d[i])-Ef(d[i-1]))/(S0-.5*(Sff(d[i])+Sff(d[i-1])))
    
    if( 200*(d[i]-dn)/(d[i]+dn) < 1 ){
        break
    }else if(i == length(d)){
        stop("Reached maximum iterations")
    }
    
}

slope <- seq(0,S0*diff(range(x[1:i])),,length(x[1:i]))

plot(x[1:i]/1000,d[1:i]+slope*d[1:i],type='l',xlab='Distance Upstream [km]',ylab='Depth [m]')