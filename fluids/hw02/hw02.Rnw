...
$$\boxed{\delta_0^{\mathrm{crit}}=\Sexpr{round(delta_crit(d0_sub),3)}}$$
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
<<p4, fig=T,tikz=T,external=T,height=5>>=
source('4.R')
@
...
(i) We know that the flow through the constriction is critical so from the calibration curve we know $d_2$ is approximately 1.3 m and from the froude number $Q$ is approximately 9.3.
