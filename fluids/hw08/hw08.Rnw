...
<<p5>>=
	...
	r <- seq(0,1.2,,1000)
	g <- p <- matrix(NA,ncol=length(t),nrow=length(r))
	surf <- matrix(NA,ncol=1,nrow=2*length(r)) 
	rownames(g) <- rownames(p) <- r
	colnames(surf) <- paste("t = ",t[3])
	colnames(g) <- colnames(p) <- paste("t = ",t)
	...
	g <- melt(g); p <- melt(p); surf <- melt(surf)
