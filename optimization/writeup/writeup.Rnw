...
A town of Thirstyville is considering the use of groundwater from a local aquifur to augment their water supply \autoref{fig:site}.  The aquifur is approximately rectangular.  It is bounded on the east and west by river and on the north and south by impermeable boundaries. 
...
\item Environmental regulations require a minimum head in the aquifer. 
\end{itemize}
...
$$0\leq x\leq L$$
$$h(0)=1,\,\,\,\,\,h(L)=1$$
where $T$ is the transmissivity, $h$ is the head, the $Q_i$'s are pumping rates and $\delta(\cdot)$ is the Dirac delta function \citep{Wang1982}. The given boundary conditions are implicitly constant in time. The parameter values are
\begin{center}
\begin{tabular}{cc}
\toprule
Parameter & Value\\
...
As the simulation model was used to determine optimal pumping rates and well placement given certain constraints.  The optimization problem is 
...
Three hypothetical cases were considered with two sub-cases, 
\begin{enumerate}
...
For each case the sub case was considered in which (a) the maximum pumping rate ($Q^*$) is limiting and (b) the maximum pumping rate is not limiting. 
...
The results are composed of the head field and pumping rates at what I will call the {\it Maximum Satisfiable Demand} (MSD).  This is the greatest demand that can be satisfied while still meeting the constraints.  This is discussed in the next section. 

\subsection{Case 1(a)}
First consider case 1(a) when boundary conditions are constant and equal and the pumping rate is limiting.  In this case since the boundary conditions are the same for each array, the results for each array are the same (both heads and pumping rates) (\autoref{fig:results1a}). Most of the pumping occurs at the outermost node on either site and all of the pumping occurs at the outer most two nodes. The pumping at the outer two most nodes is constrained by the maximum pumping rate (2.5) and so the remaining pumping (0.4) is delegated one node inward on either side.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Case 2(a)}
In this case boundary conditions are equal on each side but decrease linearly downstream. The pumping rate is limiting.  In this case the maximum pumping occurs in the array with the highest boundary conditions, and successively less pumping occurs at we move down stream (\autoref{fig:results2a}). Only at the most upstream location (with the highest boundary condition) does pumping occur on any other node but the outer most.  No pumping occurs on the farthest downstream array because the head is already at the minimum head level. 
...
\subsection{Case 1(b)}
First consider case 1(a) when boundary conditions are constant and equal and the pumping rate is not limiting. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Case 2(b)}
In this case boundary conditions are equal on each side but decrease linearly downstream. The pumping rate is not limiting.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Case 3(b)}
In this case boundary conditions increase logarithmically on the west side and decrease linearly downstream on the east side. The pumping rate is not limiting.

\begin{figure}[!ht]
\setcounter{subfigure}{0}
\centering
\subfigure[Head (units of length)]{%
	\includegraphics[width=4in]{../optimize/figs/head-log-bounds.eps}}
\subfigure[Pumping Rates]{%
...
Solving for $D^*$ is an iterative process requiring multiple runs of the original optimization model. The MSD for each case is shown in table \ref{msd}.  We can verify that the MSD is met by adding up the pumping rates.  Take case 1(a) for example, $\sum Q_i = 2.5 * (2 edges)*(4 rows) + 0.4 (2 edges)*(4 rows) = 23.2$.  \autoref{fig:zvd} shows the relationship between demand and optimal objective function.  The relationship is nearly linear except close to $D*$ in the pumping limited cases. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Appendix A - Source Code and Sample output}
\small
