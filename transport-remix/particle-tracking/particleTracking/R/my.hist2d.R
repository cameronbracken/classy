hist2d <- function (x, y = NULL, nbins = 200, same.scale = FALSE, na.rm = TRUE, 
    show = TRUE, col = c("black", heat.colors(12)), ...) 
{
    ...
    m <- matrix(0, nrow = nbins[1], ncol = nbins[2], dimnames = list(levels(index.x), 
        levels(index.y)))
    for (i in 1:length(index.x)) m[index.x[i], index.y[i]] <- m[index.x[i], 
        index.y[i]] + 1
    xvals <- x.cuts[1:nbins[1]]
    yvals <- y.cuts[1:nbins[2]]
    if (show) {
		require(fields)
        image.plot(xvals, yvals, m, col = col, ...)
	}
