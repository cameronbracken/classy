...
for(i in 1:3){
	image.plot(interp(lon.pac-360,lat.pac,pac$eigv[,i]),
		main=sprintf("Eigen Vector %d",i),legend.mar=6.5)
	map('world',add=TRUE)
}
...
\begin{figure}[!ht]
<<pc-cor, echo=T, fig=T, tikz=T, cache=T,width=7,height=3.5,keep.source=T>>=
layout(rbind(c(1,2)))
plot(pac$pc[,1],usa$pc[,1],xlab='sst',ylab='pdsi')
lines(lfpc1,xlab='sst',ylab='pdsi')
plot(pac$pc[,2],usa$pc[,2],xlab='sst',ylab='pdsi')
lines(lfpc2,xlab='sst',ylab='pdsi')
