...
<<source,fig=F,echo=F,results=hide,cache=F>>=
	...
	setwd('src')
		...
		if(file.exists('output/lf.Rdata')){
			load('output/lf.Rdata')
		}else{
			source('readData.R')
		}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Problem 1 (iii) and (vi)}

The function \texttt{solve.yw()} was created to solve for the $AR$ parameters of any order. The results for the $AR$ fitting are shown in \autoref{ar-par}.
...
\begin{figure}[!h]
\centering
<<arma-best, echo=T, fig=T,tikz=T,external=T,height=3.5>>=

	layout(rbind(c(1,2)))
	print(best.arma.aic)
	print(best.arma.gcv)
	
	plot(gcvs,xlab='',pch=20)
	text(1:length(gcvs),gcvs,names(gcvs),pos=3,offset=2,srt=90)
 	plot(aics,xlab='',pch=20)
	text(1:length(aics),aics,names(aics),pos=3,offset=2,srt=90)
	

@
\caption{$AIC$ and $GCV$ as a function of $p$ and $q$. }
\end{figure}
...
boxplot(stats$mean,main='Mean')
	lines(x.mean,col='red')
	points(13,mean(x.ts.ann),col='red')

boxplot(sqrt(stats$var),main='Standard Deviation')
	lines(x.sd,col='red')
	points(13,sd(x.ts.ann),col='red')

boxplot(stats$lag1,main='Lag1 Correlation')
	lines(x.lag1,col='red')
	points(13,mylag(x.ts.ann,1,docor=TRUE),col='red')

boxplot(stats$skew,main='Skew',ylim=range(range(stats$skew),range(x.skew)))
...
\begin{figure}[!h]
\fbox{\parbox{\textwidth}{\small
<<3-code-seqpeak,echo=F,fig=F,external=F,results=tex,cache=F>>=
	highlight('src/3-seqpeak.R',renderer=renderer_latex(document=F))
@
}}
\caption{Code to perform sequential peak analysis. }
\end{figure}

\begin{figure}[!h]
\centering
<<arma-seqpeak, echo=T, fig=T, tikz=T, cache=T,width=4,height=4, keep.source=F>>=

#par("mar"=c(5.1,4.1,7.1,4.1))   # increase margin space for extra axes
boxplot(sim.sy,cex=.3,ylab="Storage",xlab="Yield")
lines(1:length(y),s,col='red',lwd=2)
#labs <- c((0:50)/10)
...
\begin{figure}[!h]
\centering
<<arma-box, echo=T, fig=T, tikz=T, cache=T, keep.source=T>>=
