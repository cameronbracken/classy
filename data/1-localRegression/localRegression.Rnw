...
<<anova, fig=F, echo=T, keep.source=T>>=


anova.locfit <- function(fit,y){
	SST = sum((y - mean(y))^2) 
	yhat = y - residuals(fit)
	SSR = sum((yhat - mean(y))^2) 
	SSE = sum(residuals(fit)^2) 
	MSR = SSR / length(fit$vnames)            
	MSE = SSE/(length(y) - fit$dp["df2"])    
}

print(anova(lmfit.jan))
print(anova(lmfit.jul))
print(anova(datfit.jan))
print(anova(datfit.jul))
...
@
\caption{Precipitation error map}\label{image-error}
\end{figure}
...
my.plot <- function(y,fit,main,mode=c('full','dropone')){
	...
	miss <- !is.na(y.jan)
	plot(y.jan[miss], predict(datfit.jan)[miss], 
		xlab="Observed", 
...
my.plot(y.jan, datfit.jan, 'January', mode = 'full')
my.plot(y.jan, fitcv.jan, 'January', mode = 'dropone')
my.plot(y.jul, datfit.jul, 'July', mode = 'full')
my.plot(y.jul, fitcv.jan, 'July', mode = 'dropone')

@
\caption{validation}\label{validation}
\end{figure}
