...
\begin{figure}[!ht]
\centering
<<image-locfit, fig=T, tikz=T, external=T, echo=T,cache=T,height=3.9>>=
...
\begin{figure}[!ht]
\centering
<<image-lm, fig=T, tikz=T, external=T, echo=T,cache=T,height=3.9>>=

par(mar = c(5, 4, 2, 2) + 0.1)
image.plot(dem.y,dem.x,lmgrid,col=topo.colors(nbcol),xlab='Longitude',ylab='Latitude')
...
\begin{minipage}{.5\textwidth}
\centering
<<persp-locfit, fig=T, tikz=T, external=T, echo=T, cache=T, keep.source=T, width=3.5, height=3.5>>=

par(mar=c(0,1,0,0))

persp(dem.y, dem.x, locfitgrid,
	col=color[colf(locfitgrid)],
	xlab='Longitude',
	ylab='Latitude', 
	zlab = 'Rainfall',
...
@
\caption{Local fit}\label{lfimage}
\end{minipage}
\begin{minipage}{.5\textwidth}
\centering
<<persp-lm, fig=T, tikz=T, external=T, echo=T, cache=T, keep.source=T, width=3.5, height=3.5>>=

par(mar=c(0,1,0,0))
persp(dem.y, dem.x, lmgrid,
	col=color[colf(lmgrid)],
	xlab='Longitude',
	ylab='Latitude', 
	zlab = 'Rainfall',
	theta=30, phi=30, expand=0.5,
...
@
\caption{Linear Fit}\label{lfimage}
\end{minipage}
\end{figure}
...
\begin{minipage}{.5\textwidth}
\centering
<<fullpred, fig=T, tikz=T, external=T, echo=T, cache=T, keep.source=T, width=3.5, height=3.5>>=

par(mar = c(4, 4, 2, 2) + 0.1)
plot(y, predict(datfit), 
	xlab="Observed", 
	ylab="Estimates using full data")
abline(0,1)
@
\caption{}\label{fullpred}
\end{minipage}
\begin{minipage}{.5\textwidth}
\centering
<<dropone, fig=T, tikz=T, external=T, echo=T, cache=T, keep.source=T, width=3.5, height=3.5>>=

par(mar = c(4, 4, 2, 2) + 0.1)
plot(y, predict(fitcv), 
	xlab="Observed", 
	ylab="Drop-One Estimates")
abline(0,1)
@
\caption{}\label{dropone}
\end{minipage}
\end{figure}
