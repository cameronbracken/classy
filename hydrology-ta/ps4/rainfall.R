...
if(!file.exists('colo_precip.dat')) 
	download.file('http://cires.colorado.edu/~aslater/CVEN_6833/colo_precip.dat','colo_precip.dat')
	
if(!file.exists('colo_dem.dat')) 
	download.file('http://cires.colorado.edu/~aslater/CVEN_6833/colo_dem.dat','colo_dem.dat')
...
pdf('cdf.pdf')
	plot(sort(id1),q1,type='n',xlim=c(0,max(id1)), 
		xlab='Rainfall Intensity [in/hr]', ylab = 'Excedance Probability')
	grid()
	lines(sort(id1),q1,type='s',col=1)
	lines(sort(id6),q6,type='s',col=2)
	lines(sort(id24),q24,type='s',col=3)

	legend('bottomright',c('1 hr', '6 hr', '24 hr'),lty=1,col=1:3)
dev.off()
...
pdf('idf.pdf')
	plot(c(1,6,24),idf[1,],ylim=range(idf),xlim=c(1,24),
		...
		main = 'IDF curves')
	grid(10)
	lines(c(1,6,24),idf[1,],col=1)
	lines(c(1,6,24),idf[2,],col=2)
	lines(c(1,6,24),idf[3,],col=3)
	lines(c(1,6,24),idf[4,],col=4)
	legend('bottomleft',c('2 yr', '10 yr', '25 yr', '100 yr'),lty=1,col=1:4)
dev.off()
