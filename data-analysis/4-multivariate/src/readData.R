...
lf <- lf * 0.0004690502; gr <- gr* 0.0004690502

lf.ann <- apply(lf,1,mean); gr.ann <- apply(gr,1,mean)
lf.may <- lf[,5]; gr.may <- gr[,5]
lf.mean <- apply(lf,2,mean); gr.mean <- apply(gr,2,mean)
lf.sd <- apply(lf,2,sd); gr.sd <- apply(gr,2,sd)
lf.skew <- apply(lf,2,skew); gr.skew <- apply(gr,2,skew)
